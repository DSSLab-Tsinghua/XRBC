...
parse_log_and_write_results() {
    ...
    log_info "Parsing log files from directories 16 to 46 for date: $current_date"
    ...
    # Loop through directories 16 to 36
    for dir_num in {16..46}; do
    ...
    local num_nodes_A_E1=$(grep -oP 'Number Of Nodes \(Group A\):\s*\K\d+'  ../resultE1.txt)
    local avg_cross_E1=$(grep -oP 'Average \{Cross\} Latency \(ms\):\s*\K\d+' ../resultE1.txt)
    
    # Write results to file
    cat > "$result_file" << EOF
E2:
Number Of Nodes (Group B): $num_nodes_B
Number of Request: $num_requests
Average {Cross} Latency (ms) when Number of Nodes (Group A) is $num_nodes_A: $avg_cross
Average {Cross} Latency (ms) when Number of Nodes (Group A) is $num_nodes_A_E1: $avg_cross_E1

EOF
    
    log_success "Results written to $result_file"
...
main() {
    ...
    #t=5(n=16), f=10(m=31)
    ./autoConfig.bash 16 31 100 etc/conf.json
    ...
    local current_date=$(date +%Y%m%d)
    local eva_log_file="../var/log/16/${current_date}_Eva.log"
