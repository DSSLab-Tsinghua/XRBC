...
func (q *Queue) Deserialize(input []byte) error {
	q.Lock()
	defer q.Unlock()
	return msgpack.Unmarshal(input, &q)
}
...
func (q *Queue) AppendAndTrimToMaxSize(item []byte) {
	q.Lock()
	defer q.Unlock()
	q.Q = append(q.Q, pb.RawMessage{Msg: item})
	if len(q.Q) > maxSize {
		q.Q = q.Q[len(q.Q)-maxSize:]
	}
}
...
func (q *Queue) GrabWithMaxLenAndClear() []pb.RawMessage {
	q.Lock()
	defer q.Unlock()
	if len(q.Q) > config.MaxBatchSize() {
		ret := q.Q[:config.MaxBatchSize()]
