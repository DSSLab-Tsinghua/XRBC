...
func PrintLog(verbose bool, logType TypeOfLog, msg string){
	...
	if logOpt == 0{
		...
		case DynLog:
			if DynLogger == nil{
				fpath = fmt.Sprintf(homepath+"/var/log/%s/", id)
				fileName = fmt.Sprintf("%s_Dyn.log",time.Now().Format("20060102"))
				if !IsExist(fpath) {
					err := CreateDir(fpath)
					if err != nil{
						log.Printf("[WriteLog Error] create log file error!",err)
				...
				fmt.Println("open ",fpath+fileName)
				Dyn_LogFile, err = os.OpenFile(fpath+fileName,os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
				if err != nil {
					log.Printf("[WriteLog Error] open log file error!", err)
					return
				}
				DynLogger = log.New(Break_LogFile,"",log.LstdFlags)
			}
			DynLogger.Println(msg)
		case InfoLog:
			if Info_LogFile == nil{
				fpath = fmt.Sprintf(homepath+"/var/log/%s/", id)
