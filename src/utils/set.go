...
/********Byte and Int set**********/
type EpochMsgQueue struct {
	m []struct { // 存储 msg 和 epoch 的列表，按插入顺序
		msg   []byte
		epoch int
	}
	order []int // 按插入顺序存储 epoch
	sync.RWMutex
}

// 创建新的队列
func NewEpochMsgQueue() *EpochMsgQueue {
	return &EpochMsgQueue{
		m: []struct {
			msg   []byte
			epoch int
		}{},
...
// 添加 msg 和 epoch
func (q *EpochMsgQueue) AddItem(msg []byte, epoch int) {
	q.Lock()
	defer q.Unlock()

	// 如果 epoch 已存在于 order 中，则跳过
	...
	// 添加到 m 和 order
	q.m = append(q.m, struct {
		msg   []byte
		epoch int
	}{msg: msg, epoch: epoch})
	q.order = append(q.order, epoch)
}

// 获取并删除最前面的 msg 和 epoch，但不修改 order
func (q *EpochMsgQueue) GetAndRemove() ([]byte, int, bool) {
	q.Lock()
	defer q.Unlock()
	...
	// 获取最前面的 msg 和 epoch
	item := q.m[0]
	q.m = q.m[1:] // 从 m 中移除最前面的元素

	return item.msg, item.epoch, true
}

// 获取队列长度
func (q *EpochMsgQueue) Len() int {
	q.RLock()
	defer q.RUnlock()
	return len(q.m) // 返回 m 的长度
}

// 清空队列
func (q *EpochMsgQueue) Clear() {
	q.Lock()
	defer q.Unlock()
	q.m = []struct {
		msg   []byte
...
// 返回队列中的所有内容
func (q *EpochMsgQueue) GetAllItems() ([]struct {
	msg   []byte
	epoch int
}, []int) {
	q.RLock()
