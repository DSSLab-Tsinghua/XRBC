...
func GenSig(id int64, msg []byte) []byte {
	if PriKey == nil {
		log.Println("Private key is not initialized")
		return nil
	}
...
func LoadPrivKeyFromFile(id int64) {
	...
	if err != nil {
		p := fmt.Sprintf("[crypto-ecdsa]:LoadPrivKeyFromFiles open priv file error! errorinfo:%v\n", err)
		logging.PrintLog(false, logging.ErrorLog, p)
		return
	}
	var errr error
	...
	if errr != nil {
		log.Printf("[crypto-ecdsa] conversion error %v", err)
		return
	}

	// p := fmt.Sprintf("[crypto-ecdsa]:LoadPrivKeyFromFiles load private key success! id:%v\n", id)
